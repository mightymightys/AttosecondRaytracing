...
class Fourrier(Defect):
    def __init__(self, Support, RMS, slope=-2, smallest=0.01, biggest=10):  # The sizes are the wavelength in mm
        ...
        FFT = np.zeros_like(D)
        FFT[mask] = D[mask] ** slope * np.exp(b)
        FFT = np.sqrt(FFT)

        def tiled(x):
            return np.block([[np.transpose(np.rot90(x)), x], [np.rot90(x, k=2), np.transpose(np.rot90(x, k=-1))]])

        phases = np.random.uniform(0, 2 * np.pi, size=FFT.shape)
        phase = np.block(
            [[np.transpose(np.rot90(phases)), phases], [np.rot90(phases, k=2), np.transpose(np.rot90(phases, k=-1))]]
        )
        FFT_tiled = tiled(FFT)
        FFT_tiled = FFT_tiled * np.exp(1j * phase)
        kX2 = np.linspace(-k_max, k_max, num=ResX * 2)
        kY2 = np.linspace(-k_max, k_max, num=ResY * 2)
        kXX2, kYY2 = np.meshgrid(kX2, kY2)
        DerivFFTX_tiled = FFT_tiled * 1j * kXX2
        DerivFFTY_tiled = FFT_tiled * 1j * kYY2

        deformation = np.fft.irfft2(np.fft.fftshift(FFT_tiled), FFT_tiled.shape)
        DerivX = np.fft.irfft2(np.fft.fftshift(DerivFFTX_tiled), DerivFFTX_tiled.shape)
        DerivY = np.fft.irfft2(np.fft.fftshift(DerivFFTY_tiled), DerivFFTY_tiled.shape)

        X = np.linspace(-rect[0], rect[0], num=kX2.shape[0])  # TODO why
        Y = np.linspace(-rect[1], rect[1], num=kY2.shape[0])
