...
class Fourrier(Defect):
    ...
    def __init__(self, Support, RMS, slope=-2, smallest=0.01, biggest=10):  # The sizes are the wavelength in mm
        ...
        kXX, kYY = np.meshgrid(np.linspace(0, k_max, num=ResX, dtype='float32'), np.linspace(0, k_max, num=ResY, dtype='float32'), sparse=True)
        ...
        def tiled(x):
             return np.block(
                 [[np.transpose(np.rot90(x)), x], [np.rot90(x, k=2), np.transpose(np.rot90(x, k=-1))]]
                 )        
        ...
        print("Elapsed: %s s" % (perf_counter() - _tstart_stack.pop()))
        print(self.rms)
