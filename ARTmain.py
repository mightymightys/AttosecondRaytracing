...
def load_config(config_file):
    ...
    try: DetectorOptions = config.DetectorOptions
    except: 
        print("No DetectorOptions-dictionary provided, will use defaults.")
        DetectorOptions = {}
    
    try: AnalysisOptions = config.AnalysisOptions
    except: 
...
def setup_detector(OpticalChain, DetectorOptions, RayList= None):
    ...
    if DetectorOptions['ManualDetector']:
        if DetectorOptions['DetectorCentre'] is None:
            raise RuntimeError('For manual detector placement you need to specify "DetectorCentre" in the "DetectorOptions"-dictionary.')
        if DetectorOptions['DetectorNormal'] is None:
            raise RuntimeError('For manual detector placement you need to specify "DetectorNormal" in the "DetectorOptions"-dictionary.')
        Detector = mdet.Detector(OpticalChain.optical_elements[DetectorOptions['ReflectionNumber']].position, DetectorOptions['DetectorCentre'], DetectorOptions['DetectorNormal'])
    else:
        if DetectorOptions['DistanceDetector'] is None:
            raise RuntimeError('For automatic detector placement you need to specify "DistanceDetector" in the "DetectorOptions"-dictionary.')
        if RayList is None:
            raise RuntimeError('For automatic detector placement you need to add a RayList as an input (selected from the "RayListHistory" by the index DetectorOptions["ReflectionNumber"]).')
...
def optimize_detector(RayListAnalysed, Detector, DetectorOptions, verbose=True, maxRaystoConsider=1000, IntensityWeighted=False, Amplitude=None, Precision=3):
...
def run_ART(OpticalChain, SourceProperties, DetectorOptions, AnalysisOptions, loop = False):
    ...
    if AnalysisOptions["verbose"]:
        ...
        if OpticalChain.loop_variable_name is not None and OpticalChain.loop_variable_value is not None:
            print("For " + OpticalChain.loop_variable_name + " = " + '{:f}'.format(OpticalChain.loop_variable_value) + ":\n")
...
def main(OpticalChainList, SourceProperties, DetectorOptions, AnalysisOptions, save_file_name = None):
    ...
    SourceProperties, DetectorOptions, AnalysisOptions = complete_defaults(SourceProperties, DetectorOptions, AnalysisOptions)
